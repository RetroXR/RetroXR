...
- ~45 suites (`--list` names them): netplay, romm, binding, object_sync, scene, poster, rope,
  av, system, link, state, expansion, card, deck, microphone, famicom, n64_vru, n64_cart, gb_cart,
...
- `rope_bench --settle` (`still_awake=0`) and `rope_stress` are BIT-EXACT oracles — an
...
| 2e | `gb-link.md` | **Every handheld lead has its OWN plug family** (`LinkPlug`/`LinkPort.plug_family`): GB+GBA `link_plug`, and `gg_link_plug` / `ws_link_plug` / `comlynx_plug` / `ngp_link_plug`, each keyed on the machine scene AND the lead. They were one group once, so every lead seated in every handheld and only the wire objected, in a log line. `link_tests` walks the lead x socket matrix; the "find every link lead" sweeps use `LinkPlug.ANY_GROUP`, never a family. gambatte and mGBA share wire `gb-sio-1`. Drive the two machines one at a time; Tetris DX is not a usable target. Game Gear: genesis_plus_gx on `gg-ext-1` (UART + parallel pins, byte-time delivery, no framing error with no cable); Faceball/MK/MK2 need the two machines powered on at different times. |
| 2e′ | `ws-link.md` | WonderSwan Communication Cable: our `mednafen_wswan` fork speaks `ws-sio-1` (RetroArch#19454 API), a byte stamped at its stop bit; `LinkPort` on `wonderswan.tscn`, lead is `ws_link_cable`. Game probe: one game per process, with a `--nocable` control leg. |
| 2e-sat | `saturn-link.md` | Saturn Link Cable: our `mednafen_saturn` fork (Beetle) emulates the SH-2 SCI — it never existed — on wire `saturn-sci-1`, **slave SH-2 only** (a master-line crossover hangs both at boot). Bytes announced at shift START; DRCR RXI/TXI now pace DMA (Daytona). Link games: Steeldom, GunGriffon II, Daytona **Circuit Edition (JP)**, Doom, Gebockers, Hyper Reverthion — NOT Hyper Duel / Virtual On. `saturn_link_*` extends `psx_link_*`, own plug group. `SS_SCI_TRACE` floods stall the core. **Netplay (fork past v2): rollback, and a cabled pair rolls back as a group** — `link_frame_edges` gives every frame one span on the LINK CLOCK (not fixed `retro_run` windows), `netplay_deterministic` fixes the SMPC clock; `saturn_rollback_probe` / `saturn_session_probe`. Console lead plugs (`RcaPlug`s) must join `LinkPlug.ANY_GROUP` or netplay never finds the far machine. |
| 2e‴ | `lynx-link.md` | ComLynx: our `mednafen_lynx` fork speaks `comlynx-1`, 2–8 units on the GBA lead's shape (`comlynx_cable`, its junction chains more). A byte lands at its STOP BIT, a unit's own echo goes through the shared inbox, overlapping frames AND — each one was a game failing without it. **Probes stagger power-on** (identical units on identical clocks collide on every byte: received == sent is the tell) and run ONE at a time. BIOS in `system/mednafen_lynx/`. Gauntlet and California Games still start alone. **Netplay rolls a cabled pair back as one group** only with `lynx_fixed_frames` (fork v2+): constant 213333-cycle frames so every unit's frame edge is one bus tick; the latch waits for every peer past the byte (it used to depend on which thread arrived first); the savestate carries Mikey's RX queue and screen-DMA position (`MIKR`) and the link (`LINK`). `lynx_rollback_probe` / `lynx_session_probe`. |
| 2e″ | `ngp-link.md` | SNK link cable (Neo Geo Pocket/Color): our `mednafen_ngp` fork speaks `ngp-sio-1` at the 6.144 MHz CPU clock, bytes paced at 19200 baud. **NeoPop had INTRX0/INTTX0 swapped (RX is 11/0x6FE4)**; the inbox doubles as the BIOS receive buffer; 0xB2 is /RTS out, /CTS in, sent across as its own message. `LinkPort` on `neo_geo_pocket.tscn`, lead is `ngp_link_cable`. Press the two units APART; run game probes one at a time. |
| 2e-ps2 | `ps2-ilink.md` | PS2 i.LINK: our PS2 fork attaches port 0, `ps2-ilink-1394`, a real 1394 bus of up to 64 (six verified on GT3). `ilink_cable` + a six-port `ilink_hub`; **no lead decides its own bus** — `ILinkBus` walks leads through hubs (chained hubs are one bus) and calls `LinkConnectGroup` per set, parting stale orphans FIRST. `ILinkPlug` is in `any_link_plug` (PS1/Saturn/Jaguar plugs are not). GT3 needs a reset AFTER its driver starts: replug a lead if a console was cabled before it booted. **Three-screen Broadcast verified through the room** (`Tools/link/gt3_ilink_hub_probe` + `gt3_three_screen.txt`, windowed): Console ID 1 is the control unit, IDs 2/3 its left/right views; a console bounced out of i.LINK Battle goes back in with CROSS + a replug. |
...
| 2g | `netplay.md` | A link cable never crosses the network: every machine on every connected wire is in the session; all three lead types answer `linked_machines()`; a plug seated mid-game lands on ONE host-agreed frame. dolphin is DETERMINISM-only (no state transfer). **§2g′: a cabled group whose cores end frames on one bus tick (`link_rollback`) rolls back TOGETHER** (`NetplayGroupIteration`: every member stops at every frame edge, earliest mismatch wins, bus captured/restored with the cores); `power_on_stagger` switches units on apart and holds the room lead off (`holds_cable`) until a scheduled join. Late join into a rolling group is refused until measured. | **PlayStation (pcsx_rearmed fork, §2g″): a cabled pair rolls back as a group on desktop AND Quest** (262/262 vs lockstep) -- but only because every load is exact (SIO1, memcard FLAG, gpulib write cache/GPUREAD/VRAM transfer, five upstream fields), a byte never lands past the last GRANT, and `pcsx_rearmed_netplay_deterministic` pins every worker thread off (a Quest's defaults parted two LOCKSTEP runs at frame 120). `psx_rollback_probe`.
...
| 2q | `n64-cartridges.md` | `N64CartShell` (header CRC + market, never hashed), `CartridgeColor`, flake shader. Bodies carry no marks. Run `Tools/glb/fix_unmapped_uvs.py` on any re-exported body; never `demetal` this model. `HoldPress` spawn sub-menu (`_show_cart_spawn_options`). |
...
| 2w | `rope.md` | Cables. **A loose plug is Jolt's: the rope never writes its transform** — `CouplePlug` pushes it with forces from the solver's corrections at the pinned end (`end_align_stiffness` > 0 is now just the on switch); the old per-tick `AlignAnchorPlug` turn is what made cord ends squirm on the floor. **Plug colliders are boxes fitted to the connector, never spheres** (a sphere rolls and props the plug 3 cm up). **Friction is Coulomb on VELOCITY only** — three position-level versions each broke something, see the table; keep static ≤ ~1.5× kinetic and kinetic > 0. A loose plug is reeled in ONLY through `PlugTether.reel_in` (swept, kills the velocity it undoes, 5 mm slack) — never a hand-written clamp; the sixteen old copies disagreed and one let a plug fight a sleeping cord for ever. Self-collision never pushes a strand INTO the surface it rests on, is INELASTIC (moves the Verlet history with every correction AND stops the approach), and lifts back out only what it pushed in — or a loop crossing itself on the floor pops strands over each other. The rope cannot hold a plug's weight; the host clamp does. `floor_ends_probe` / `plug_hang_probe`. Headset feel OWED. |
| 2x | `gb-cartridges.md` | `GbCartShell` (header title + destination + CGB flag, never hashed) picks grey/black/Pokemon colours; `CartridgeColor` takes a system id, `SHADE` + `OWN_ROUGHNESS` for the GB mouldings. Same `HoldPress` sub-menu as N64, no Body row. Body carries no marks. Run `Tools/glb/fix_unmapped_uvs.py` on any re-export. |
| 2y | `gba-cartridges.md` | `GbaCartShell` (maker `01` + game code, never hashed): Ruby/Sapphire/Emerald/FireRed/LeafGreen spawn in CLEAR plastic, everything else solid grey. `CartridgeShellPreset.opacity` < 1 (or a colour's alpha) → the two-pass clear shader (MULTIPLY filter + ADDITIVE gloss: order-independent, never an alpha blend); at 1 the plain material. The GLB is Codex's tintable export copied in unchanged (shells export solid; photographed PCB inside, CC BY-SA 4.0 photos credited in About). |
