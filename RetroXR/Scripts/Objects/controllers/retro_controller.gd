...
# The accessory port on the back of the pad, and what is in it. Inert on every
# controller whose scene does not author an ExpansionPort node, which is all of
# them but the N64's. Shared with PadReceiver, which has one too — see
...
func _on_pak_changed() -> void:
	if _pak_port.get_pak() == null:
		_apply_rumble()
...
## The core option value this controller's port should take. "" means this pad
...
func _apply_rumble() -> void:
	...
	if _pak_port.has_port() and not is_instance_valid(_pak_port.get_pak()):
		_rumble.weak = 0.0
		_rumble.strong = 0.0
