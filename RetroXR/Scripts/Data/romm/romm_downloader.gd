...
func _worker(args: Dictionary) -> void:
	...
	var outcome := _transfer(args, dest, rom_id)
	...
	var owned := PackedStringArray([fs_name])
	...
	if bool(entry.get("multi", false)):
		...
		members.append({"path": fs_name, "size": ByteSize.on_disk(dest)})
	...
	_register_group.call_deferred(systemid, fs_name, launch_relative,
...
func _transfer(args: Dictionary, dest: String, rom_id: int) -> Dictionary:
	var part := dest + ".part"
	var attempt := 0
	var last_error := ""
	...
	while attempt < MAX_RETRIES:
		...
		match str(outcome["status"]):
			...
			"terminal":
				return outcome
			"unranged":
				# Not a failure: go again at once, from zero, without a Range.
...
## ok / transient / restart / unranged / terminal / cancelled.
func _attempt_download(args: Dictionary, dest: String, part: String, rom_id: int) -> Dictionary:
	...
	if not bool(args.get("unranged", false)):
	...
	match result:
		...
		RommHttp.Result.HTTP_ERROR:
			# RomM answers a ranged request for a multi-file ROM from a zip it caches
			# as root with mode 0600, which its own nginx workers cannot read: 403.
			# Without a Range the same URL streams the archive instead.
			if code == 403 and bool(entry.get("multi", false)) \
					and not bool(args.get("unranged", false)):
				args["unranged"] = true
				return {"status": "unranged", "error": ""}
...
func _extract_multi(zip_path: String, systemid: String,
	...
	for item: Dictionary in inspected.get("files", []):
		...
		elif ext in ["cue", "ccd", "mds", "iso", "chd", "gdi"]:
			discs.append(entry_name)
	# RomM adds an .m3u to every multi-file download. For one disc it names only
	# that disc, and not every disc core reads a playlist: virtualjaguar does not.
	if discs.size() == 1 or (launch_entry.is_empty() and not discs.is_empty()):
		launch_entry = discs[0]
