...
# is needed beyond naming the core. card must be forced to genesis: the
# default fallback in card_systemid is the unit's OWN media systemid, which
# is mastersystem, and a player who owns a Genesis is not looking on the
