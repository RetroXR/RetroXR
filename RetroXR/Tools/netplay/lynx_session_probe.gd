## A real netplay SESSION over a ComLynx cable: two players, two Lynxes each.
##
##   "$godot" --headless --path RetroXR res://Tools/netplay/lynx_session_probe.tscn -- \
##       [--mode=rollback|lockstep] [--rom=<.lnx>] [--frames=N]
##
## A probe, not a test: real mednafen_lynx cores (RetroXR's fork), the Lynx BIOS
## and a commercial ROM (default Warbirds). One process, never two at once: it
## listens on a fixed loopback port.
##
## lynx_rollback_probe proves the ENGINE (Wrapper::NetplayGroupIteration) with
## the inputs fed by hand. This is everything around it, the way players reach
## it: two NetworkManagers over real loopback ENet, each peer with its own room
## -- two Lynxes built from system.tscn and the spawn catalog's ComLynx Cable
## pushed into their sockets -- and the host starting a session on one of them.
## The session decides the strategy, forms the group, switches the units on
## apart, joins the cable on its scheduled frame, and carries every press
## between the peers. The host's player has unit 0, the client's unit 1; each
## presses only their own.
##
...
func _ready() -> void:
	...
	get_tree().create_timer(300.0).timeout.connect(func() -> void:
		print("[lynxnp] TIMEOUT")
		get_tree().quit(1))
	await _run()


## Warbirds' two-player start, the sequence docs/dev/lynx-link.md gives: both
## players through the briefing, and in flight by about frame 2725.
func _press(machine: int, frame: int) -> int:
	var presses: Array = [
		[0, 1537], [1, 1759], [0, 2081], [1, 2403],
		# The session joins the lead a few frames later than a room does, which
		# moves the briefing by a press; each player confirms once more.
		[0, 2600], [1, 2700],
	]
...
func _run() -> void:
	if not FileAccess.file_exists(_rom):
		print("[lynxnp] SKIP no ROM at %s" % _rom)
		get_tree().quit(0)
		return
	...
	for b: Dictionary in [host, client]:
		...
		np.desync_detected.connect(func(_peer: int, frame: int) -> void:
			_desyncs += 1
			print("[lynxnp] DESYNC reported by %s @frame %d" % [b["name"], frame]))
	...
	if not await _until(func() -> bool: return hnp.is_running() and cnp.is_running(), 1200):
	...
	_check("the cabled units roll back together" if _mode == "rollback" else "with the units staggered",
		(hnp._group_rollback and cnp._group_rollback) if _mode == "rollback" else (hnp._staggered and cnp._staggered))
	for bi in range(2):
		...
		if still > 3000:
			for b: Dictionary in [host, client]:
				for unit: RetroSystem in b["units"]:
					...
					print("[lynxnp] wedge %s %s: frame %d identity %s stats %s" % [b["name"], unit.name,
	...
	for b: Dictionary in [host, client]:
		for unit: RetroSystem in b["units"]:
			...
			print("[lynxnp] %s %s: frame %d, rollbacks %d, sent %d, heard %d, peers %d" % [
...
## The spawn catalog's ComLynx Cable, pushed into both units' sockets.
func _cable(b: Dictionary) -> void:
	...
	if scene == null:
		_check("the spawn catalog has a ComLynx Cable", false)
		return
	...
	for unit: RetroSystem in units:
		ports.append(unit.find_child("LinkPort", true, false))
...
func _shot(host: Dictionary, client: Dictionary) -> void:
	...
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://probe_out/lynx/session"))
	out.save_png("res://probe_out/lynx/session/%s.png" % _mode)


func _check(what: String, ok: bool, detail := "") -> void:
	print("[lynxnp] %s  %s%s" % ["ok  " if ok else "FAIL", what, "" if detail.is_empty() else " (" + detail + ")"])
	if not ok:
		_fail += 1
...
func _finish(why: String) -> void:
	if not why.is_empty():
		_check(why, false)
	print("[lynxnp] %s (%s)" % ["PASS" if _fail == 0 else "FAIL %d" % _fail, _mode])
