## A Communication Cable between two WonderSwans, seated the way a hand seats it.
##
##     "$godot" --headless --path RetroXR res://Tools/link/ws_link_room_probe.tscn [-- --roms=Z:/roms]
##
## ws_link_game_probe joins two cores with LinkConnect and drives real games over
## the result. This drives the path a PLAYER uses: two WonderSwans built by the
## room, a lead from the spawn catalog, plugs pushed into each unit's EXT socket.
## The lead goes in before either unit is switched on, as the box says, and Puyo
## Puyo Tsuu's boot handshake -- one byte each way -- is the traffic that proves
## the seated lead carries something. It needs the retroXR build of
## mednafen_wswan; the stock core never receives a byte.
extends Node3D

const SYS_SCENE := "res://Scenes/Objects/system.tscn"
const CORE := "mednafen_wswan"
const GAME := "Puyo Puyo Tsuu (Japan).ws"
...
func _run() -> void:
	var rom := _find_rom()
	if rom.is_empty():
		print("[ws-room] SKIP  no %s; pass --roms=<library>" % GAME)
		get_tree().quit(0)
		return
	...
	if not FileAccess.file_exists(root.path_join("cores").path_join(CORE + "_libretro.dll")) \
		...
		print("[ws-room] SKIP  the %s core is not installed" % CORE)
		get_tree().quit(0)
		return

	for i in range(2):
		var sys := (load(SYS_SCENE) as PackedScene).instantiate() as RetroSystem
		sys.systemid = "wonderswan"
		sys.core_name = CORE
		sys.name = "WS%d" % i
		add_child(sys)
	...
	_ok("the WonderSwan model carries an EXT socket", ports.count(null) == 0)
	...
	var entry := {}
	for item in SpawnCatalog.items_for("wonderswan"):
		if str(item.get("label", "")) == "Communication Cable":
			entry = item
	_ok("the spawn menu offers a Communication Cable", not entry.is_empty())
	...
	var got0 := _traffic(0)
	var got1 := _traffic(1)
	print("[ws-room] delivered %d / %d" % [got0, got1])
	_ok("the boot handshake crosses the seated lead both ways", got0 > 0 and got1 > 0,
...
func _find_rom() -> String:
	...
	for root in roots:
		var path := root.path_join("wonderswan").path_join(GAME)
...
func _ok(name: String, cond: bool, detail := "") -> void:
	if cond:
		_pass += 1
		print("[ws-room] PASS  %s" % name)
	else:
		_fail += 1
		print("[ws-room] FAIL  %s%s" % [name, "  — " + detail if not detail.is_empty() else ""])
...
func _finish() -> void:
	...
	for sys in _systems:
		if is_instance_valid(sys):
			sys.power_off()
	await _settle(90)
	print("[ws-room] ---- %d passed, %d failed ----" % [_pass, _fail])
