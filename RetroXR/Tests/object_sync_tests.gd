...
func _test_registry_lifecycle() -> void:
	...
	sync._apply_event(NetEvents.Event.EV_DISK_OP, {
	...
	sync._update_port_owner(NetEvents.Event.EV_PORT_PLUG,
		{"sys": card, "ctrl": fixed_control, "port": 2}, 7)
	sync._update_port_owner(NetEvents.Event.EV_PORT_UNPLUG, {"sys": card, "port": 2}, 7)
...
func _test_controller_end_to_end(p: Pair) -> void:
	...
	p.host_os.report_event(NetEvents.Event.EV_PORT_PLUG,
	...
	p.host_os.report_event(NetEvents.Event.EV_PORT_UNPLUG,
...
func _test_events(p: Pair) -> void:
	...
	p.client_os.report_event(NetEvents.Event.EV_SYS_POWER, {"sys": client_obj})
	_ok(await _until(func() -> bool: return host_obj.power_toggles == 1),
		"events/a client's platform power button executes on the host")
	p.host_os.report_event(NetEvents.Event.EV_SYS_POWER_STATE,
	...
	p.client_os.report_event(NetEvents.Event.EV_SYS_RESET, {"sys": client_obj})
	...
	p.host_os.report_event(NetEvents.Event.EV_TV_SIZE, {"tv": host_obj, "scale": 1.35})
	_ok(await _until(func() -> bool:
	...
	p.host_os.report_event(NetEvents.Event.EV_SYS_VIDEO_OUT,
		{"sys": host_obj, "on": false})
	p.host_os.report_event(NetEvents.Event.EV_SYS_GRAVITY,
		{"sys": host_obj, "on": true})
	_ok(await _until(func() -> bool:
		return not client_obj.video_out and client_obj.floating),
		"events/platform video-output and gravity toggles take effect")
	p.host_os.report_event(NetEvents.Event.EV_ROOM_LIGHTS,
		{"switch": host_obj, "on": false})
	p.host_os.report_event(NetEvents.Event.EV_PULL_LIGHT,
		{"cord": host_obj, "on": false})
	p.host_os.report_event(NetEvents.Event.EV_BLINDS,
		{"blinds": host_obj, "drop": 0.25})
	p.host_os.report_event(NetEvents.Event.EV_TIME_OF_DAY,
	...
	p.host_os.report_event(NetEvents.Event.EV_TRAY, {"sys": host_obj, "open": true})
	...
	p.client_os.report_event(NetEvents.Event.EV_BOOK_PAGE,
	...
	p.host_os.report_event(NetEvents.Event.EV_BOOK_SIZE,
		{"book": host_obj, "scale": 1.6})
	p.host_os.report_event(NetEvents.Event.EV_BOOK_HALF,
	...
	p.host_os.report_event(NetEvents.Event.EV_DVD_INSERT,
		{"dvd": host_obj, "disc": host_aux})
	p.host_os.report_event(NetEvents.Event.EV_AUDIO_INSERT,
	...
	p.host_os.report_event(NetEvents.Event.EV_DVD_REMOVE, {"dvd": host_obj})
	p.host_os.report_event(NetEvents.Event.EV_AUDIO_REMOVE, {"player": host_obj})
	_ok(await _until(func() -> bool:
	...
	p.host_os.report_event(NetEvents.Event.EV_CART_REMOVE, {})
	p.host_os.report_event(NetEvents.Event.EV_TAPE_REMOVE, {"vcr": null})
	...
	p.host_os.report_event(NetEvents.Event.EV_TV_PLUG,
		{"owner": host_obj, "tv": host_aux})
	p.host_os.report_event(NetEvents.Event.EV_TV_UNPLUG,
		{"tv": host_obj, "in": 3})
	p.host_os.report_event(NetEvents.Event.EV_RCA_PLUG,
		{"cable": host_obj, "end": 1, "cord": 2, "dev": host_aux, "port": "VideoIn"})
	p.host_os.report_event(NetEvents.Event.EV_RCA_UNPLUG,
	...
	for cmd: String in ["play", "pause", "stop", "ff", "rew"]:
		p.client_os.report_event(NetEvents.Event.EV_VCR_CMD,
	...
	for cmd: String in ["play", "pause", "stop", "menu_up", "menu_down", "menu_left",
			...
			"subtitle"]:
		p.client_os.report_event(NetEvents.Event.EV_DVD_CMD,
			{"dvd": client_aux, "cmd": cmd})
	for cmd: String in ["play", "pause", "stop", "ff", "rew", "next", "prev"]:
		p.client_os.report_event(NetEvents.Event.EV_AUDIO_CMD,
