...
| `gba_cart_tests` | 60 | 5 s | the Game Boy Advance cartridge: the tintable model imported solid, which surfaces are moulding, colour with baked roughness kept, the five clear Pokemon shells (filter + surface passes, colour, opacity, normal map, order-independent blends) and back to solid, the parts no colour reaches (label, screw, and the photographed board, contact strip, chips and battery, all solid; board and strip share the photos, at most 1024 wide), the header lookup (game code by market, maker `01` only), spawned cartridges, and a shell forced from the spawn menu |
