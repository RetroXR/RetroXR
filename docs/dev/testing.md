...
| `gba_cart_tests` | 57 | 5 s | the Game Boy Advance cartridge: the tintable model imported solid, which surfaces are moulding, colour with baked roughness kept, the five clear Pokemon shells (filter + surface passes, colour, opacity, normal map, order-independent blends) and back to solid, the parts no colour reaches, the header lookup (game code by market, maker `01` only), spawned cartridges, and a shell forced from the spawn menu |
