...
| a saved room | `systemid` and `cart_systemid` through `canonical` at deserialize, and `rom_path` through `RomLibrary.relocate`, which finds a ROM whose folder was since renamed. **Not** by bumping `ScenePersistence.VERSION`, which drops the slot. |
